...
bot.command('wallets', async (ctx) => {
  ...
  const lines = rows
    .map(
      (r) =>
        `${r.id}. ${r.name} — ${r.address}${u?.active_wallet_id === r.id ? ' (active)' : ''}`,
    )
    .join('\n');
...
bot.command('price', async (ctx) => {
  ...
  const amountInWei = ethers.parseEther('1'); // 1 WPLS path
  try {
...
bot.action(
  'settings',
  (ctx) => ctx.reply('Use /set_token <addr> and /set_gas <priority> <max> <limit>'),
);
